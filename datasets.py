...
def create_dataloader_custom(path, imgsz, stride, opt , batch_size, tile_size , overlap, 
    ...
    batch_size = 32 # min(batch_size, len(dataset)) 
...
class load_custom(Dataset):
    ...
    def __getitem__(self, index):
            ...
            #print(shape)
            # coords = self.batch_coords[self.batch[index]] if self.rect else self.img_size  # final letterboxed shape
            img, ratio, pad = letterbox(img, self.img_size, auto=False)#, scaleup=self.augment)
            shapes = (h0, w0) #, ((h / h0, w / w0), pad)  # for COCO mAP rescaling            
            #coords  = coord # ndpa coords
            img = img[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB, to 3x416x416
            img = np.ascontiguousarray(img)
            return torch.from_numpy(img), self.img_files[index], shapes , coord     
        
    def collate_fn(batch):
            img, indices, shapes , coords = zip(*batch)  # transposed            
...
def load_tile(self, index):
    ...
    #img = np.ascontiguousarray(img)
    coord = tile_source.get_tile_coordinates(tile_source.level_count-1, tile_id)            
    ...
    return img, (h0, w0), img.shape[:2], coord   # img, hw_original, hw_resized
    # else:
...
def create_dataloader(path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
    ...
    print('lentght of dset', len(dataset))
